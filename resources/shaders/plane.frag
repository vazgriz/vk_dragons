...
layout(set = 3, binding = 0) uniform sampler2D textureColor;
layout(set = 3, binding = 1) uniform sampler2D textureNormal;
layout(set = 3, binding = 2) uniform sampler2D textureEffects;
layout(set = 3, binding = 3) uniform samplerCube textureCubeMap;
layout(set = 3, binding = 4) uniform samplerCube textureCubeMapSmall;
...
vec3 shading(vec2 uv, float lightShininess, vec3 lightColor, out vec3 ambient){
    ...
    vec3 d = normalize(lightUniforms.position.xyz - lightUniforms.position.w * Inposition);
...
void main(){    
    ...
    vec3 ambient;
    vec3 lightShading = shading(parallaxUV, lightUniforms.shininess, lightUniforms.Is.rgb,ambient);
    ...
    vec3 lTangentDir = normalize(IntangentSpaceLight - lightUniforms.position.w * IntangentSpacePosition);
    ...
    fragColor = vec4(ambient * lightUniforms.Ia.rgb + shadowMultiplicator * lightShading, 0.0); 
}
