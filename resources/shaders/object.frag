...
void main(){
    ...
    if(diffuse > 0.0){
        vec3 r = reflect(-d,n);
        specular = pow(max(dot(r,v),0.0), lightUniforms.shininess);
        specular *= effects.g;
    }
    ...
    if(effects.b > 0.0){
        ...
        rCubeMap = vec3(camUniforms.viewInverse * vec4(rCubeMap,0.0));
        reflectionColor = texture(textureCubeMap,rCubeMap).rgb;
    }

    vec3 lightShading = diffuse * diffuseColor + specular * lightUniforms.Is.rgb;
    ...
    vec3 fColor = ambient * lightUniforms.Ia.rgb + shadow * lightShading;
